...
print(f"[INFO] Original Audio Length: {len(y)/fs:.2f}s, Sample rate: {fs} Hz")

# Force the audio length to 21 seconds
target_duration = 21  # seconds
target_length = target_duration * fs

if len(y) < target_length:
    # Pad with zeros if audio is shorter than 21 seconds
    y = np.pad(y, (0, target_length - len(y)))
else:
    # Trim if audio is longer than 21 seconds
    y = y[:target_length]

print(f"[INFO] Adjusted Audio Length: {len(y)/fs:.2f}s")
...
# Trim to original length (21 seconds)
